                        ...
                        <form method="POST" action="{{ route('register') }}" enctype="multipart/form-data">
                                ...
                                <div class="col-md-6 dir-ltr">
                                    <div class="input-group mb-3">
                                        <div class="input-group-prepend">
                                          <select name="state_id" @click.self="directSelectState($event)" class="custom-select mr-sm-2 dir-rtl open" ref="states" id="city">
                                              <option value="" {{ old('state_id') ? '' : 'selected' }} v-if="userState.length==0">استان ها</option>
                                              <option  v-for="(state,index) in fStates" :key="index" :value="state.id" @click="selectState(state)">@{{state.state}}</option>
                                          </select>
                                        </div>
                                        <input name="state" value="{{old('state')}}" type="text" @keyup.self="filterStates" v-model="userState" class="form-control" aria-label="Text input with dropdown button" ref="state">
                                      </div>
                            ...
                            <div class="form-group row ">
                                ...
                                <div class="col-md-6 dir-ltr">
                                    <div class="input-group mb-3">
                                        <div class="input-group-prepend">
                                          <select name="city_id"  @click="directSelectCity($event)" class="custom-select mr-sm-2 dir-rtl open" ref="cities" id="state">
                                              <option value="" {{ old('city_id') ? '' : 'selected' }} v-if="userCity.length==0">استان ها</option>
                                              <option  v-for="(city,index) in fCities" :key="index" :value="city.id" @click="selectCity(city)">@{{city.city}}</option>
                                          </select>
                                        </div>
                                        <input name="city" type="text"  @keyup="filterCities" value="{{old('city')}}" v-model="userCity" class="form-control" aria-label="Text input with dropdown button" ref="city">
                                      </div>
